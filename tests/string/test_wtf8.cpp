...
#include <s2/detail/encoding_wtf8.h>
...
#define CASE(x, t) \
        TEST_CASE("WTF8 check " #x " is valid " #t, "[string/wtf8]") \
        { const uint8_t* v = (const uint8_t*)x; \
          REQUIRE(s2::encoding::wtf8::validate(v, v + sizeof(x) / sizeof(x[0]) - 1) == t); \
        }

#define VALID(x) CASE(x, true)
#define INVALID(x) CASE(x, false)

TEST_CASE("WTF8 check writing and reading of characters") {
  s2::basic_string<s2::encoding::utf16> u16s(u"uÜℵ💩"); // 1, 2, 3 and 4 byte characters. u, U umlaut, aleph and poop.
  s2::basic_string<s2::encoding::wtf8> u8s = u16s.view();
...
// WTF8 differs in that it considers this okay.
VALID("\xED\xA0\x80"); // Invalid surrogate half 0xD800
VALID("\xED\xAF\xBF"); // Invalid surrogate half 0xDBFF
VALID("\xED\xB0\x80"); // Invalid surrogate half 0xDC00
VALID("\xED\xBF\xBF"); // Invalid surrogate half 0xDFFF
